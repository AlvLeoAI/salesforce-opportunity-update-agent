...
|---|---|
| Hallucinated quote — model invents a timestamp/quote that isn't in the transcript | Two-stage check in `schema_validator`: an exact substring match first, then a windowed fuzzy match (≥80% of quote tokens present in a sliding window) to tolerate light LLM paraphrasing. Quotes that fail both are stripped from the evidence; if a populated draft field has no surviving evidence after stripping, the field's value is cleared too (`stage` is the exception — no null sentinel, so we keep the value with a loud `evidence_filter:` warning). |
| Inferred field with no quote — model populates `stage` from context (procurement + pricing → `negotiation`) without citing a specific turn | Treated as advisory, not blocking. `validate_evidence_coverage` now appends a `field 'X' has no grounding quote; value was inferred by the model` warning instead of raising. The result is returned as-is; the reviewer sees the inferred value plus a clear UI warning about which fields are not transcript-anchored. The principle: any schema-compliant result reaches the reviewer, evidence gaps surface as warnings, never as a 422. |
